...
import algorithm.utility.BinaryHeap;
import algorithm.utility.PriorityQueue;
...
public class PathFinder {
	...
	private GridNode jump(GridNode curNode, int dx, int dy) {
		...
		if (dx != 0 && dy != 0) {
			// If neighbors do exist and are forced (left and right are
			// impassable)
			...
			if (jump(nextNode, dx, 0) != null || jump(nextNode, 0, dy) != null)// Special
																				// Diagonal
																				// Case
				return nextNode;
		...
		return jump(nextNode, dx, dy); // No forced neighbors so we are
